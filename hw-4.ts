...
type Genres = 'Action' | 'Sci-Fi' | 'Adventure' | 'Drama' | 'Fantasy' | 'Family'
...
// 1. Собрать в массив все жанры фильмов (без повторения)
const buildArrayWithGenre = (films: Films): String[] => {
    let newArray = films.filter(item => item.genre)
        .map(item => item.genre.join(' '))
        .join(' ')
        .split(' ')
    return [...new Set(newArray)]
}
console.log(buildArrayWithGenre(films))

// 2. Собрать в массив всех актеров всех фильмов (без повторения)
const buildArrayWithActors = (films: Films): String[] => {
    let newArray = films
        .filter(item => item.actors)
        .map(item => item.actors.join(', '))
        .join(', ')
        .split(', ')
    return [...new Set(newArray)]
}
console.log(buildArrayWithActors(films))

// 3. Отсортировать фильмы по рейтингу по убыванию
function getSortByImdbRating(films: Films): Films {
    return films.sort((a, b) => b.imdbRating - a.imdbRating)
}
console.log(getSortByImdbRating(films))

// 4. Создать новый массив, где объекты фильмов будут состоять из следующих полей: id, title, released, plot
// function buildNewArray(films) {

//     return films.map(item => item.id + item.title + item.released + item.plot).split()


// }
// console.log(buildNewArray(films))